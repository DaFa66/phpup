...
- **Delete removes the apt MariaDB-generated `feedback.cnf`** — `feedback=OFF` is unknown to MySQL 26+ (aborts on start), so the file left behind in `/etc/mysql/mariadb.conf.d` would break a foreign MySQL on the same box after a phpup reinstall. Deleted with the stack; other `/etc` configs remain untouched.
